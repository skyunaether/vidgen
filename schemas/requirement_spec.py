...
class AcceptanceCriteria(BaseModel):
    must_have: List[str] = Field(default_factory=list, description="Measurable pass/fail criteria")
    nice_to_have: List[str] = Field(default_factory=list, description="Desirable but not strictly required")
...
class RequirementSpec(BaseModel):
    ...
    narrative_beats: List[str] = Field(default_factory=list, description="Key story beats to hit")
    forbidden_content: List[str] = Field(default_factory=list, description="Things to strictly avoid")
    cta_overlay_rules: Optional[str] = Field(None, description="Rules for text overlays or CTA at the end")
    audio_requirements: str = Field(default="Background music matching the mood, balanced volume", description="Audio specs (music presence, ducking rules)")
